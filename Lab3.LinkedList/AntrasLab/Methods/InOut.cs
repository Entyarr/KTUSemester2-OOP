...
namespace AntrasLab.Methods
{
    ...
    public class InOut
    {
        ...
        public static LinkedList<Agent> ReadAgents(Stream stream)
        {
            using (StreamReader read = new StreamReader(stream))
            {
                ...
                while ((line = read.ReadLine()) != null)
                {
                    ...
                    agents.Add(new Agent(agentCode, lastName, name, address, number));
        ...
        public static LinkedList<Customer> ReadCustomers(Stream stream)
        {
            using (StreamReader read = new StreamReader(stream))
            {
                ...
                while((line = read.ReadLine()) != null)
                {
                    ...
                    customers.Add(new Customer(address, lastName, number, start, end, subCode, magCount, agentCode));
                }
                return customers;
            }
        }
        ...
        public static void MonthInQuestion(string fileName, int month)
        {
            using (StreamWriter writer = new StreamWriter(fileName, true))
            {
                writer.WriteLine("Menuo, kuris nagrinejamas: " + month);
                writer.WriteLine();
            }
        }
        /// <summary>
        ...
        public static void PrintCustomers(LinkedList<Customer> customers, string fileName, string header)
        {
            using (StreamWriter writer = new StreamWriter(fileName, true))
            {
                writer.WriteLine(new string('-', 104));
                writer.WriteLine("| {0, -100} |", header);
                writer.WriteLine(new string('-', 104));
        ...
        public static void PrintAgents(LinkedList<Agent> agents, string fileName, string header)
        {
            using (StreamWriter writer = new StreamWriter(fileName, true))
            {
        ...
        /// <param name="fileName">File name</param>
        /// <param name="header">Title of box</param>
        public static void PrintMasterList(LinkedList<CustomersByAgent> agents, string fileName, string header)
        {
            using (StreamWriter writer = new StreamWriter(fileName, true))
            {
